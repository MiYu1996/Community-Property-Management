...
const GET_PURE = (url: string) => new Request(url, { method: 'GET' })
const GET_URL_PARAM = (url: string, param: string) => new Request(`${url}/${param}`, { method: 'GET' })
const POST_OBJ = (url: string, body: any) => new Request(url, { method: 'POST', body: JSON.stringify(body) })
const POST_URL_PARAM = (url: string, param: PostUrlFormatter) => new Request(param.format(url, param.param), { method: 'POST', body: JSON.stringify(param.body) })
