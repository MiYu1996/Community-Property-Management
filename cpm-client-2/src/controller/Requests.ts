...
const GET_PURE = (url: string) => new Request(formatUrl(url), { method: 'GET',  headers:{'Content-Type' : 'application/json', 'Accept' : 'application/json' }})
const GET_URL_PARAM = (url: string, param: string) => new Request(formatUrl(`${url}/${param}`), { method: 'GET',  headers:{'Content-Type' : 'application/json' , 'Accept' : 'application/json' }})
const POST_OBJ = (url: string, body: any) => new Request(formatUrl(url), { method: 'POST', headers:{'Content-Type' : 'application/json', 'Accept' : 'application/json' },  body: JSON.stringify(body) })
const POST_URL_PARAM = (url: string, param: PostUrlFormatter) => new Request(param.format(url, param.param), { method: 'POST',  headers:{'Content-Type' : 'application/json', 'Accept' : 'application/json' }, body: JSON.stringify(param.body) })
const POST_REPLY = <T>(url: string, param: PostObjParam<T>) => POST_URL_PARAM(url, { format: (url, param) => formatUrl(`${url}/${param}/reply`), ...param })
const POST_COMMENT = <T>(url: string, param: PostObjParam<T>) => POST_URL_PARAM(url, { format: (url, param) => formatUrl(`${url}/${param}`), ...param})
...
const [announcementPostSubject, announcementPostFetch$] = rxOneToOne<AnnouncementPostRequest>("/api/announcement", POST_OBJ, true)
...
const [discussionPostSubject, discussionPostFetch$] = rxOneToOne<DiscussionPostRequest>("/api/discussion", POST_OBJ, true)
